...
check from verifying `?` carrier propagation and
...
Generic invocation runtime coverage is active in `GenericInvocationCodeGenTests`.
It checks nullable generic ABI calls, nullable `ToString` results, boxed variadic
arguments and spread contents (including empty inputs), and static abstract
interface parsing with both valid and invalid input. Opcode and intermediate
collection assertions have been replaced by observable results. Run this slice with
`dotnet test test/Raven.CodeAnalysis.Tests/Raven.CodeAnalysis.Tests.csproj --filter 'FullyQualifiedName~.GenericInvocationCodeGenTests.' /property:WarningLevel=0`.

Lambda runtime coverage is active in `FunctionExpressionCodeGenTests`. It
checks parameter, local, and instance captures; shared mutable state across
lambdas and local functions; recursive named lambdas; and synchronous and async
iteration. Duplicate closure tests and generated-class layout assertions have
...
Primary-constructor runtime coverage is active in
`PrimaryConstructorParameterCodeGenTests`. It checks record overrides, private
constructor metadata, and async access to constructor state and method parameters
after controlled suspension, including success and error payloads. Run this slice with
`dotnet test test/Raven.CodeAnalysis.Tests/Raven.CodeAnalysis.Tests.csproj --filter 'FullyQualifiedName~.PrimaryConstructorParameterCodeGenTests.' /property:WarningLevel=0`.

Source-versus-metadata precedence coverage is active in
`TypeResolutionPrecedenceTests`. It checks source-union symbol identity and
case conversions, then executes construction and matching when an imported
assembly defines a union with the same name and arity. Run this slice with
`dotnet test test/Raven.CodeAnalysis.Tests/Raven.CodeAnalysis.Tests.csproj --filter 'FullyQualifiedName~.TypeResolutionPrecedenceTests.' /property:WarningLevel=0`.

Async propagation, try expressions, and runtime-async backend coverage are active
in `AsyncPropagateCodeGenTests`, `AsyncTryAwaitCodeGenTests`,
`TryExpressionCodeGenTests`, and `RuntimeAsyncCodeGenTests`. Runtime checks cover
error payloads, generic arrays, async entry-point exit codes, and lambda results;
optional IL-verifier gates and helper-call assertions no longer stand in for
those behavior checks.
...
| Runtime CodeGen | stale runtime/reflection/emitted-shape classes are excluded from `scripts/test-runtime-isolated.sh`: `AttachedMacroCodeGenTests`, `PdbSequencePointTests`, `ProjectFileNuGetReferenceTests`, `PropertyTests`, `RuntimeSymbolResolverTests`, `UnionCodeGenTests` | Reintroduce only focused runtime behavior checks; avoid emitted instruction/lowered shape assertions |
